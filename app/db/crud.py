...
def get_russian_ingredients_from_db():
    project_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "../.."))  # Путь до корня проекта
    db_path = os.path.join(project_dir, "app", "db", "ingredients.db")
    ...
    try:
        connection = sqlite3.connect(db_path)
        cursor = connection.cursor()
        cursor.execute("SELECT id, traditional_name FROM ingredients WHERE traditional_name IS NOT NULL")
        ingredients = [(row[0], row[1]) for row in cursor.fetchall()]
        connection.close()
        return ingredients
    except sqlite3.OperationalError as e:
...
def get_latin_name(ids):
    ...
    try:
        ...
        query = f"""
        SELECT latin_name, traditional_name, danger_factor
        FROM ingredients
        WHERE id IN ({', '.join(map(str, ids))})
        """
